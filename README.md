...
Tenant directory and provisioning are implemented and wired end-to-end: `knightage-identity` calls this service on registration, this service creates and schemas a real per-tenant database for each of Accounting, CRM, and Inventory & Sales, and records the mapping. A minimal Angular ops UI (login + a Tenants page) lets an operator see provisioned tenants and their per-service databases, and manually trigger/retry provisioning.

**Deferred, not part of this pass:** the business apps (`knightage-accounting`, `knightage-crm`, `knightage-inventory-sales`) still each connect to one static database via their own `ConnectionStrings:Default`, regardless of which tenant is calling. Making each of them resolve and connect to the correct tenant-specific database per request (e.g. from the JWT's `org_id` claim, looked up via this service's `GET /api/tenants/{organizationId}`) is real multi-tenant request routing and is a separate follow-up slice — provisioning the isolated databases (this repo's job) is a prerequisite for that, not a substitute.
